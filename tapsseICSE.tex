...
Still, given a $\mathit{m_i}$, we first seek a prior event $\mathit{m_j}$ from 
$\pi$ who might let $\mathit{m_i}$ trigger a cache hit. That is, the addresses 
$\mathit{a_j}$ and $\mathit{a_i}$ have the same \emph{tag} and \emph{set} values
(cf. \circled{4}). Moreover, we need to find the nearest $\mathit{m_j}$ because 
$\mathit{m_i}$'s cache behavior directly relates to it. This demand is accomplished
by ensuring the non-existence of an event $\mathit{m_x}$ between $\mathit{m_j}$ and 
$\mathit{m_i}$. Also, $\mathit{m_x}$ and $\mathit{m_i}$ have to own the same 
\textit{tag} and \textit{set} values (cf. \circled{5}).
