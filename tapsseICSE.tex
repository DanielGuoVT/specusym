...
under speculative execution, $\mathit{m_i}$ must cause either new opposite 
behavior or new divergent behavior. In any case, it is a leak. Then the 
solving of $\theta$ turns to be solving the existence of such $\mathit{in}$, 
which has cheaper computation cost. Thereby we define a new constraint
$\theta'$ as follows.  
%
\begin{multline}
  \label{eqn:leak}
...
memory value rather than a cache value. This assumption indeed requires a heavy 
cache analysis whereas~\cite{WuW19} over-approximately treats all accesses as 
...
examination results in a cache hit, we know that speculative analysis at this 
point is unnecessary. Otherwise, we conduct $\theta$ for analysis as usual. 
