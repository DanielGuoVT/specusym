...
\begin{itemize*}
  ...
  \item $\mathit{\mu_{in}(m_i)}$ denotes the condition that $\mathit{m_i}$ 
	can trigger a cache hit under speculative execution with 
    input $\mathit{in}$. 
...
$\mathit{\mu_{in}(m_i)}$. E.g., for \texttt{S[x]} \textit{read} in 
Figure~\ref{fig:3a}, $\mathit{\mu_{in}(m_i)}$ can be satisfied but 
$\mathit{\eta_{in}(m_i)}$ cannot.
...
To analyze this behavior, we build $\mathit{\mu_{in}(m_i)}$ which returns the 
cache hit condition of $\mathit{m_i}$ in trace $\pi$:
%
%
\begin{multline}
  \label{eqn:hc}
...
after interpreting a memory event (line 14). In this procedure, \SpecuSym first 
examines whether the current state $s$ is a $\mathit{symbolic}$ state, and the 
event $s.e$ relates to sensitive input. It is because the timing effect inside 
speculative execution is externally invisible, and we are interested in the 
...
To form $\theta$, we have to build the constraint exposing new cache behaviors
...
define $\theta:=\mathit{div_i}\vee\mathit{opp_i}$ as the leak constraint at $\mathit{m_i}$.
...
of $\theta$, \SpecuSym generates a test case including the inputs, the trace $M$, and 
...
constraints to $\theta$ for analysis.  
...
simplify $\theta$ into $\theta'$ as follows.  
%
\begin{multline}
  \label{eqn:leak}
  ~~~~~~~~~~~~~~~
...
either \textit{Divergent} or \textit{Opposite}. Thereby $\theta'$ conforms to 
$\theta$. Also, since we repeatedly traverse memory access trace in constraint 
