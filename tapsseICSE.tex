...
in symbolic execution as a three-step analysis and evaluate its cache side effects 
...
\textit{SpeculativeExplore}. Usually, at a conditional branch (line 8), we 
call \textit{SpeculativeExplore} (line 9) to start the speculative probe. 
That is, if the branch predicate relies on memory visit (line 21), e.g.,
using a variable for the first time, we duplicate a new state $s'$ from 
current state \textit{s} (line 22) with a negated branch direction (line 23). 
This assumption is from the observation that the memory visiting latency 
at a branch may create a time window for speculative execution to load data 
into cache before the CPU rollback. Otherwise, despite a branch misprediction, 
CPU may spend several cycles on speculative execution, which is too quick to 
let speculation affect the cache.
