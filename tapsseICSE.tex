...
its parent state. For clarity, we use \textit{speculative} state to alias the new 
state. A \textit{speculative} state would be prioritized to the front of the state 
container in symbolic executor, and uninterruptedly executes until it meets a 
...
from its parent state, owing to the \textit{copy-on-write} schema in modern symbolic 
executors~\cite{CadarDE08,CiorteaZBCC09}. The memory updates in a \textit{speculative} 
state won't taint its parent state who waits at the state-forking point for the 
\textit{speculative} state returns. For the sake of cache analysis, we also maintain 
the cache data in each symbolic state. A \textit{speculative} state inherits such data 
from the parent state and keeps updating the cache during its execution.
