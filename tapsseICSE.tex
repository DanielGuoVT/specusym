...
\begin{abstract}
  CPU cache is a limited but crucial storage component in modern processors, 
	whereas the cache timing side-channel may indirectly leak data through the 
	measurable timing variances. Speculative execution, a vital microarchitectural
  ...
  This work proposes a new approach, \textit{Speculative symbolic Execution} (SSE), 
	...
	effects along subsequent explorations. Based on the dynamic information and a 
	specified cache model, SSE constructs leak predicates for memory accesses	and 
	conducts a constraint-solving based cache behavior analysis to generate leak 
	witnesses.
  %
  We built a tool named \SpecuSym atop KLEE following SSE, and evaluated it against 
	14 open-source benchmarks. Experiments show that \SpecuSym successfully	detected 
	leaks in 6 programs on 4 different cache settings and identified false positives 
