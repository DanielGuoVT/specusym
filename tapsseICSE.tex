...
\begin{abstract}
  ...
  This work proposes a new method, \textit{Speculative symbolic Execution} 
	...
	execution), \texttt{SSE} systematically explores the state space, models 
	speculative behavior at conditional branches, and accumulates the	cache 
	side effects along subsequent explorations. 
	%
	During the dynamic execution, \texttt{SSE} constructs leak predicates for 
	memory visits based on a specified cache model and conducts constraint-solving 
	based cache behavior analysis to generate leak witnesses.
  %
  We have implemented \texttt{SSE} in a tool named \SpecuSym atop KLEE and 
	evaluated it against 14 open-source benchmarks. Experiments show that 
	\SpecuSym successfully detected leaks in 6 programs upon 4 different cache 
	settings and identified false positives in 2 programs reported by a recent 
	work.
\end{abstract}
