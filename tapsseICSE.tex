...
\begin{abstract}
  CPU cache is a limited but crucial storage component in processors, whereas the 
	cache timing side-channel may inadvertently leak information by the measurable 
	timing variances. Speculative execution, a microarchitectural	optimization and 
	a source of such variances, can cause severe detriment on deliberate branch 
	mispredictions.
  %
  Despite static analysis could qualitatively verify the timing-leakage-free 
	...
	like inputs and speculated flows to diagnose leaks in depth. 
  %
  This work proposes a new method, \textit{Speculative symbolic Execution} (SSE), 
	for precisely detecting cache timing leaks introduced by speculative execution. 
	Given a program (leakage-free in non-speculative execution), SSE systematically
	explores the state space, models speculative behavior at conditional branches, 
	and accumulates the	cache side effects along subsequent explorations. 
	Then based on a specified cache model, it constructs leak predicates for memory 
	visits and conducts a constraint-solving based cache behavior analysis to generate 
	leak witnesses.
  %
  We have realized SSE in a tool \SpecuSym atop KLEE and evaluated it against 
	14 open-source benchmarks. Experiments show that \SpecuSym successfully	detected 
	leaks in 6 programs upon 4 different cache settings and identified false positives 
	in 2 programs reported by a recent work.
\end{abstract}
