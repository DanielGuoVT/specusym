...
\cite{WuW19}. For example, \SpecuSym pinpoints 9, 5, and 8 leaky memory operations 
in these benchmarks, respectively. Moreover, \SpecuSym can provide inputs and memory 
event trace for leak diagnosis. Note that \SpecuSym generally performs faster on $C_4$ 
than on set-associative caches $C_1$, $C_2$, and $C_3$ since the extreme cache setting 
type of $C_4$ advances both cache utilization and cache hit rate for these benchmarks. 
...
As stated earlier, the cache setting for this experiment is $C_3$.
