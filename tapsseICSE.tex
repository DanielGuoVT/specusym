...
\begin{abstract}
  CPU cache is a limited but crucial storage component in modern processors, 
	...
	through the measurable timing variances. Speculative execution, a 
	microarchitectural optimization and a source of such variances, can cause 
  ...
  This work proposes a new method, \textit{Speculative symbolic Execution} (SSE), 
	...
	%
	During the dynamic execution, SSE constructs leak predicates for memory visits 
	based on the specified cache model and conducts a constraint-solving based cache 
	behavior analysis to generate leak witnesses.
  %
  We have implemented SSE in a tool named \SpecuSym atop KLEE and evaluated it against 
...
Program analysis for speculative execution is by no means a new approach. Literally, 
...
\cite{LiMR03,LiMR05}, and concurrency bug prediction~\cite{ChenWYS09,LiELS05}, while
...
in depth. Moreover, the over-approximation nature inherently leads to false positives, 
which demands a more precise method.
