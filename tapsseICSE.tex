...
\begin{abstract}
  CPU cache is a limited but crucial storage component in processors, 
	whereas the cache timing side-channel may inadvertently leak data by the 
	measurable timing variances. Speculative execution, a microarchitectural
	optimization and a reason for such variances, can engender severe detriment 
	from deliberate branch mispredictions.
  %
  Though static analysis could qualitatively verify the timing-leakage-free 
	property under speculative execution, it is incapable of producing endorsements 
%	including inputs and speculated flows 
	to diagnose leaks in depth. 
  %
  This work proposes a new approach, \textit{Speculative symbolic Execution} (SSE), 
	for precisely detecting cache timing leaks introduced by speculative execution. 
	Given a program (leakage-free in non-speculative execution) and its sensitive 
	input, SSE systematically explores the program state space. Meanwhile, it models 
	speculative behavior at conditional branches, and accumulates the	cache side 
	effects along subsequent explorations. Based on the dynamic execution and a 
	specified cache model, SSE then constructs leak predicates for memory visits and 
	conducts a constraint-solving based cache behavior analysis to generate leak 
	witnesses.
  %
  We built a tool \SpecuSym atop KLEE following SSE and evaluated it against 
	14 open-source benchmarks. Experiments show that \SpecuSym successfully	
	detected leaks in 6 programs on 4 different cache settings and identified 
