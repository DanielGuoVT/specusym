...
indicates the first array read in the $\mathit{while}$ loop, and \texttt{v2} 
means the memory \emph{write} in the $\mathit{else}$ branch. The solid arrow 
line denotes the execution flow of state $s$, which takes the $\mathit{if}$ 
...
the entire cache has been filled since the memory write in $s'$ mapped \texttt{v2} 
into line \#256. 
...
\ref{fig:3b}. Also, the last memory \emph{load} to \texttt{S[x]} might 
...
Note that Figure~\ref{fig:3a} only shows the modeling of the \emph{else} 
...
\begin{figure}
\subfigure[Executions of states $s$ and $s'$]{
\label{fig:3a}
\begin{minipage}{0.4\linewidth}
    ...
    \node[above=3pt, right, scale=.8] at (0.1, 2.4) {\texttt{S[0]}};
  	\node[above=3pt, right, scale=.8] at (0.1, 2.2) {......};
		...
		\node[above=3pt, right, scale=.8] at (-0.4, 1.2) {\texttt{v1}};
    \node[above=3pt, right, scale=.8] at (0.05, 1.2) {\texttt{v2}};
