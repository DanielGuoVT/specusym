...
may also exist in speculation. As the speculative window is usually short, 
we treat the nested depth as configurable and set it to 2 in evaluation. 
Experiments demonstrate the effectiveness of this under-approximation approach.
			 

\SpecuSym works on the LLVM IR. However, the program runtime behavior might 
differ due to backend compiler optimization and address space layout 
randomization (ASLR). More low-level details can further facilitate \SpecuSym 
to improve the analysis.


\SpecuSym does not support the analysis of multithreaded programs. {\sc{SymSC}}
\cite{GuoWW18} leveraged multithreading for timing leak detection. However, 
considering multithreading and speculative execution greatly amplifies the 
state space and interleaving space, which pose exceptional challenges for a 
precise analysis.
...
cache timing attacks, which motivates the design of \SpecuSym. 


To estimate cache side channels, Doychev et al.~\cite{DoychevFKMR13}
approximate cache states by concrete state abstraction. Despite the
upper bound of information leakage, the estimation supplies guidance on 
leakage fixing. Doychev et al.~\cite{DoychevK17} also show that compiler 
optimization can remove side channels. However, it is undesirable for 
critical software to offer a compiler-dependent security guarantee. 
...
To quantify the information leakage through a cache side-channel, Chattopadhyay 
...
targets timing leaks through a trace-based symbolic execution. Furthermore, 
Brotzman et al.~\cite{BrotzmanLZTK2018} proposed CaSym, a symbolic reasoning 
method that supports cache analysis over multiple program paths. Considering
the cache effects from thread interleavings, Guo et al.~\cite{GuoWW18} proposed
\textsc{SymSC} to analyze cache timing leaks due to multithreading. These 
methods, however, are all unaware of cache state changes due to speculative 
execution.  
...
\cite{WangBLWZW19} provides a secret-augmented abstract domain to track 
program secrets and dependencies. It treats public data in a coarse-grained 
fashion and secret data in a finer-grained manner to balance the scalability 
and precision. However, CacheS is also unaware of the impact of speculative 
execution on the cache state. Though Wu et al.~\cite{WuW19} developed a 
dedicated abstract interpretation method for worst-case execution estimation 
and cache timing leak analysis under speculative execution, the method is 
incapable of generating precise inputs to diagnose leaks in depth. 
...
execution~\cite{NilizadehNP19,HeEC19} or relying on costly
instrumentations on the control flow graph. Also, due to the testing nature, 
these methods primarily count on the fuzz heuristics while \SpecuSym 
systematically solves for precise inputs.


To conclude, existing program analysis approaches either fail to generate 
inputs or ignore cache affects from speculative execution. In contrast, 
\SpecuSym models speculative execution through stateful symbolic execution 
to produce precise inputs that can study the cache timing side-channel leaks 
in depth. 
