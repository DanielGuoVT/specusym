...
Figure~\ref{fig:specu_modeling} shows the speculative modeling of program $\prog$ 
exampled in Figure~\ref{fig:motiv}. Figure~\ref{fig:fig3-a} displays the control 
flow graph of $\prog$ on which we annotate the memory access related variables. 
For example, {S[0]} means the first array read in the $\mathit{while}$ loop, and 
\texttt{v2} means the memory \emph{write} in the $\mathit{else}$ branch. The solid 
arrowed line denotes the execution flow of state $s$ which takes the $\mathit{if}$ 
