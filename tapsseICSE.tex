...
memory value rather than a cache value. Without this assumption, \cite{WuW19} over-approximately treats all accesses as memory visit and thus includes the scenarios where speculative execution is not conducted. To deal with this assumption requires a heavy cache analysis starting from the current branch to the first memory access, examining whether the value involved in the predicate is in cache or not. However, to balance the precision and efficiency, \SpecuSym instead backwardly examines the memory accesses from the current branch to the last branch. If the predicate value is in cache and not evicted in this memory access sequence, we know that cache is always hit for predicate and cache speculative analysis at this 
