...
(line 22) by invoking \textit{SpeculativeExplore} at a branch event (line 9). 
...
event $s.e$ relates to sensitive input (line 32). It is because the timing effect 
...
$\theta$ for event $s.e$ and solves for a solution (lines 33-35). If the solution
...
However, the size of $\theta'$ still increases drastically along with the 
increase of $i$, which pressures the constraint solver. By inspecting 
...
$s'$ from symbolic state $s$ (line 22) if the branch predicate relates to a
memory value rather than a cache value. This is because speculative execution is not conducted if predicate doesn't access memory. Without this assumption, \cite{WuW19} over-approximately treats all accesses as memory visit and thus includes the scenarios where predicate values are all in cache. To deal with this assumption requires a cache analysis starting from the current branch to the first memory access, examining whether the value involved in the predicate is in cache or not. This analysis needs to calculate the cache states for all possible paths and thus is very heavy. However, it's easy to observe that the memory accesses from the current branch to the last branch are included in all possible paths and have the most influence on the cache state before the branch. Therefore, to balance the precision and efficiency, \SpecuSym instead only backwardly examines the memory access in this sequence. If the predicate value is fetched to cache and not evicted in this memory access sequence, we know that cache is always hit for predicate and thus cache speculative analysis at this point is unnecessary. Otherwise, we still conduct $\theta$ for analysis as usual to avoid missing any input resulting in speculative execution at this branch. Although this optimization introduces an extra cache analysis for some cases, it's still worthy as we found that predicate values are in cache before the branch in most cases due to time and spatial locality.
...
main aspects. First, we made KLEE support bounded execution of the auxiliary 
\textit{speculative} state and schedule the parent \textit{symbolic} state 
afterwards, to mimic the branch misprediction. Second, we made KLEE stitch 
...
events to generate concrete inputs for timing leaks. Based on these new 
...
constraint, \SpecuSym outputs leak witness including the event trace, the 
leaky memory operation, and the concrete inputs. 

We design following research questions for the experiments: 
\begin{compactitem}
