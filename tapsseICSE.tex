...
\begin{abstract}
  CPU cache is a limited but crucial storage component in modern processors, 
	...
	an essential processor optimization, and a source of such variation, can cause 
  ...
  We have implemented \textsc{SpecuSym} atop KLEE and evaluated it against 15 
	...
	detected from 2 to 61 leaks in 6 programs under three different cache settings 
...
three set-associative caches and identifies false positives in 2 programs reported by 
...
control hazard, the processors first predicts a branch to take. Then, they 
...
to its parent state. For clarity, we use \textit{speculative} state to alias 
this newly forked state. A \textit{speculative} state would be prioritized to 
the front of the state container in symbolic executor, and uninterruptedly 
...
It is worth noting that we constrain the customary scope of \textit{speculative
execution} to \textit{branch prediction} and subsequent out-of-order instruction 
execution in this work. In practice, modern processors may conduct other forms of 
...
classes in modern processors. The last setting forms a fully-associative 
...
$C_3$, under which \texttt{Base} and \texttt{Opt} have a minimum performance gap, 
forming a lower-bound result. 
...
\caption{Performance comparison of the individual program. Benchmarks are listed by their orders in Table~\ref{tbl:benchs}.}
...
of speculative execution or relying on sohpisticated instrumentations on 
