...
\Begin{
    \If{$\mathit{s}$ is a regular symbolic state {\bf{and}} s.e relates to sensitive input}{
...
\SpecuSym considers nested speculative execution since branches may also 
exist on speculated paths. As the speculative window is usually transient, 
we treat the nested depth as configurable and set it to \textit{two} in 
evaluation. Also, \SpecuSym executes at most the reorder buffer size of 
instructions on a speculative path. However, experiments show the 
