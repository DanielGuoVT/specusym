...
that a sequential program $\prog$ consists of a finite set of instructions 
and $\prog$ defines the execution semantics in the program paths. Let $inst$ 
be an instruction then an event $e$:=$(l_b\succ inst\succ l_a)$ represents 
the interpretation of \emph{inst} in symbolic execution where $l_b$ and $l_a$ 
denote the program locations right before and after $inst$, respectively. A 
program execution explores a program path by interpreting a sequence of events. 
Close to~\cite{GuoKWYG15,GuoWW18}, we abstract $e$ into three categories w.r.t 
the type of the \emph{inst} that $e$ contains.
...
%Meanwhile, both \textit{if} and \textit{else} branches have to be 
%feasible at this point (line 20). 
...
Instead, \SpecuSym models all possible branch mispredictions along an execution 
path. It also maintains a \textit{stitched} trace $\mathit{M}$ of memory events  
along execution as the alternative of the cache state. Then it builds the leak 
constraint from stitched events, and lazily searches solutions to expose leaks 
by constaint solving. We detail this trace-based analysis in section~\ref{sec:modeling} 
and section~\ref{sec:analysis}. 
...
In this section we show how \SpecuSym models the cache state. The cache 
state during a program execution refers to the status of the cache lines 
after executing every memory-related instruction, e.g., the cache snapshot 
shown in Figure~\ref{fig:fig3-b}.
...
The size of equation (\ref{eqn:leak}) still increases quickly along with 
the increase of $i$, which pressures the constraint solver. By inspecting 
the formula structure, we seperate lengthy equations into smaller pieces 
by following strategies. First, if a sub-constraint of a conjunctive normal 
form (CNF) formula is determined as \textit{false}, we directly return 
\textit{false}. E.g., in equation (\ref{eqn:hc}), if \circled{1} is false, 
we skip querying solver for \circled{2}, and \circled{3}. Second, if a 
...
(\ref{eqn:hc}), once we find the nearest $\mathit{a_j}$ by \circled{2}, we 
discard all sub-CNF for indices before $j$ since they are in contradiction. 
Third, if two sub-formulas are in negated forms, we avoid a second solver 
query. E.g., in equation~\ref{eqn:hc}, constraint \circled{2} of $\mathit{m_j}$ 
is the negation of constraint \circled{1} of $\mathit{m}_{\mathit{j}+1}$, 
which implies we can save one constraint solving time. 
...
free of timing leaks hence marked with \ding{55}. For these non-leaky programs, 
...
for those \ding{52} results from~\cite{WuW19}, \SpecuSym has different results 
    ...
    \scalebox{.55}{
      \begin{tikzpicture}
        \pgfplotsset{every axis legend/.append style={
          at={(0.85,0.62)}, anchor=south},every axis y label/.append style={at={(0.09,0.5)}}}
...
\begin{table}%[!tb]
\caption{Performance Increases from Optimizations}
\vspace{-1em}
\label{tbl:opt}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}{ccccc}
\hline
\textbf{~~Optimization~~} &\textbf{~~Time (m)~~}&\textbf{~~Solved Constraints~~}&~~\#\textbf{.Div}~~&~~\#\textbf{.Opp}~~ \\ 
\hline
\texttt{Base} &~~~3600~~~ &10.15k    &5  &6\\ 
%\texttt{+S\&C} &~~~2400~~~ &2.13k  &8  &7\\ %2
%\texttt{+SCR} &~~~1200~~~ &1.24  &9  &10\\ %2
\texttt{Optimized} &~~~600~~~ &0.1k  &11 &12\\ %2
\hline
%\bottomrule
\end{tabular}
}
...
\begin{table}%[htb!]
\caption{Comparison of different \SpecuSym configurations}
\label{tbl:params}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}{rcccccc}
...
%\cline{1-7}
\multirow{2}{*}{\textbf{Name~~~~}} 		&  
\multicolumn{2}{c}{\textbf{2 mispredict}}&		
\multicolumn{2}{c}{\textbf{3 mispredict}}&		
\multicolumn{2}{c}{\textbf{4 mispredict}}\\
%\cline{2-7}
\cmidrule(r){2-3} \cmidrule(r){4-5}\cmidrule(r){6-7} 
&\#.Leaks &Time(m)  &\#.Leaks &Time(m) & \#.Leaks  &Time(m) \\ 
\midrule
%\hline
%\hline
AES\cite{OpenSSL}       	&57  &55   &430.2  &57  &0/55  &140.3\\  %8
Seed\cite{Chronos}    		&1   &0    &288.9  &1   &1/0  &41.4\\ %2
Encoder\cite{LibTomCrypt}  	&1   &0    &0.1    &1   &1/0  &0.1\\ %12
Hash\cite{LibTomCrypt}         &1   &0    &0.1    &1   &1/0 &0.1\\ %8
Chacha20\cite{Chronos}       	&1   &0    &0.1    &1   &1/0     &0.1\\   %8
\hline
Chaskey\cite{DinuBGKCP2015}     &1   &0    &0.1    &1   &1/0    &0.1\\   %8
DES\cite{Libgcrypt}   		&16  &15   &7.8    &16  &1/15    &3.5\\%8
DES\cite{Chronos}    		&1   &0    &0.1    &1   &1/0    &0.1\\ %75
FCrypt\cite{Chronos}       	&16  &15   &4.1    &16  &0/15   &8.1\\%2
Kasumi\cite{Botan} 		&1   &0    &0.1    &1   &1/0    &0.2\\ %75
\hline
Khazad\cite{Chronos}    	&25  &23   &206.5  &25  &0/23    &83.0\\ %75
fc2268\cite{Libgcrypt}     	&1   &0    &0.1    &1   &1/0    &0.1\\ %75
Seed\cite{Libgcrypt}     	&1   &0    &0.1    &1   &1/0    &0.1\\ %75
TWINE\cite{DinuBGKCP2015}       &1   &0    &0.1    &1   &1/0    &0.1\\ %75
Twofish\cite{Libgcrypt}         &1   &0    &0.1    &1   &1/0    &0.2\\ %75
\bottomrule
%\hline
\end{tabular}
}
